...
export function publishWidgetData(settings: AthanSettings, language?: AppLanguage): void {
  ...
  try {
    ...
    const values = {
      ...
      nextPrayer: prayers.find((prayer) => Number(prayer.time) > Date.now() / 1000),
    ...
    const writeAndReload = () => {
      ...
      // Use property-list primitives only. This avoids mixed Foundation/JSON
      // decoding inside the memory-constrained WidgetKit process.
      if (values.nextPrayer) {
        ...
        storage.set('nextPrayerTime', Math.floor(Number(values.nextPrayer.time)));
