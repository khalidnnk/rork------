...
const makeCities = (country: string, countryAr: string, timezone: number, timezoneId: string, cities: [string, string, number, number][]): City[] =>
  cities.map(([name, nameAr, latitude, longitude]) => ({ name, nameAr, latitude, longitude, timezone, timezoneId, country, countryAr }));

const SAUDI_CITIES = makeCities('Saudi Arabia', 'السعودية', 3, 'Asia/Riyadh', [
...
const UAE_CITIES = makeCities('UAE', 'الإمارات', 4, 'Asia/Dubai', [
...
const KUWAIT_CITIES = makeCities('Kuwait', 'الكويت', 3, 'Asia/Kuwait', [
...
const EGYPT_CITIES = makeCities('Egypt', 'مصر', 2, 'Africa/Cairo', [
...
const JORDAN_CITIES = makeCities('Jordan', 'الأردن', 3, 'Asia/Amman', [
...
const IRAQ_CITIES = makeCities('Iraq', 'العراق', 3, 'Asia/Baghdad', [
...
const PALESTINE_CITIES = makeCities('Palestine', 'فلسطين', 2, 'Asia/Hebron', [
...
const LEBANON_CITIES = makeCities('Lebanon', 'لبنان', 2, 'Asia/Beirut', [
  ['Beirut', 'بيروت', 33.8938, 35.5018],
  ['Tripoli', 'طرابلس', 34.4332, 35.8498],
  ['Sidon', 'صيدا', 33.5633, 35.3697],
]);

const SYRIA_CITIES = makeCities('Syria', 'سوريا', 3, 'Asia/Damascus', [
...
const MOROCCO_CITIES = makeCities('Morocco', 'المغرب', 1, 'Africa/Casablanca', [
...
const MAURITANIA_CITIES = makeCities('Mauritania', 'موريتانيا', 0, 'Africa/Nouakchott', [
  ['Nouakchott', 'نواكشوط', 18.0735, -15.9582],
]);

const SOMALIA_CITIES = makeCities('Somalia', 'الصومال', 3, 'Africa/Mogadishu', [
  ['Mogadishu', 'مقديشو', 2.0469, 45.3182],
  ['Hargeisa', 'هرجيسا', 9.5600, 44.0650],
]);

const DJIBOUTI_CITIES = makeCities('Djibouti', 'جيبوتي', 3, 'Africa/Djibouti', [
  ['Djibouti', 'جيبوتي', 11.5721, 43.1456],
]);

const COMOROS_CITIES = makeCities('Comoros', 'جزر القمر', 3, 'Indian/Comoro', [
